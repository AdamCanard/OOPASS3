...
public class BSTreeNode<E> {
  ...
  public void updateNode(String fileName, Integer lineNumber) {
    boolean found = false;
    Word word = (Word) this.element;

    for (Occurrence occurrence : word.getOccurrences()) {
      if (occurrence.getFileName().equals(fileName)) {
        occurrence.addLineNumber(lineNumber);
        found = true;
      }
    }
    ...
    if (!found) {
      ...
      Occurrence newOccurrence = new Occurrence(fileName, lines);
      word.addOccurrence(newOccurrence);
    }
